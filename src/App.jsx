...
const STORAGE_KEY = 'registro-docente-app-vite-v1';
...
function normalizeImportedData(parsed) {
  return {
    ...
    tasks: Array.isArray(parsed?.tasks)
      ...
      : [],
    sync: {
      deviceName: parsed?.sync?.deviceName || 'Questo dispositivo',
      shareCode: parsed?.sync?.shareCode || '',
      lastExportAt: parsed?.sync?.lastExportAt || '',
      lastImportAt: parsed?.sync?.lastImportAt || '',
    },
  };
}
...
export default function App() {
  ...
  function buildSyncPayload() {
    const payload = { ...data, sync: { ...(data.sync || {}), lastExportAt: new Date().toISOString() } };
    return JSON.stringify(payload);
  }

  function generateShareCode() {
    try {
      const encoded = btoa(unescape(encodeURIComponent(buildSyncPayload())));
      setData((prev) => ({ ...prev, sync: { ...(prev.sync || {}), shareCode: encoded, lastExportAt: new Date().toISOString() } }));
      setSyncText(encoded);
      setSyncStatus('Codice di sincronizzazione generato.');
    } catch {
      setSyncStatus('Non sono riuscito a generare il codice di sincronizzazione.');
    }
  }

  function importShareCode() {
    if (!syncText.trim()) {
      setSyncStatus('Incolla un codice di sincronizzazione.');
      return;
    }
    try {
      const decoded = decodeURIComponent(escape(atob(syncText.trim())));
      const parsed = JSON.parse(decoded);
      const normalized = normalizeImportedData(parsed);
      normalized.sync = { ...(normalized.sync || {}), lastImportAt: new Date().toISOString(), shareCode: syncText.trim() };
      setData(normalized);
      setSyncStatus('Dati importati correttamente da un altro dispositivo.');
    } catch {
      setSyncStatus('Codice non valido. Controlla di aver copiato tutto il testo.');
    }
  }

  function copySyncCode() {
    const text = data.sync?.shareCode || syncText;
    copyText(text)
      .then(() => setSyncStatus('Codice copiato negli appunti.'))
      .catch(() => setSyncStatus('Non sono riuscito a copiare il codice.'));
  }
  ...
  return (
    <div className="app-shell">
      <div className="container">
        ...
        <div className="tabs">
          {['oggi','classi','orario','lezioni','verifiche','scadenze','sync','classe'].map((t) => (
        ...
        {tab === 'sync' && (
          <TwoPanel
            left={<section className="card sticky-card"><div className="card-header"><h2><Smartphone size={18} /> Sincronizzazione tra dispositivi</h2></div><div className="card-body stack"><input className="input" value={data.sync?.deviceName || ''} onChange={(e)=>setData((prev)=>({...prev,sync:{...(prev.sync||{}),deviceName:e.target.value}}))} placeholder="Nome dispositivo" /><button className="btn" onClick={generateShareCode}><RefreshCw size={16} /> Genera codice sync</button><button className="btn btn-outline" onClick={copySyncCode}><Copy size={16} /> Copia codice</button><div className="info-box">Questa è una sincronizzazione manuale tra dispositivi: generi un codice qui, lo copi e lo incolli sull'altro dispositivo nella stessa sezione. Non serve account e non perdi l'offline.</div></div></section>}
            right={<section className="card"><div className="card-header"><h2><RefreshCw size={18} /> Importa da un altro dispositivo</h2></div><div className="card-body stack"><textarea className="input textarea tall-textarea" value={syncText} onChange={(e)=>setSyncText(e.target.value)} placeholder="Incolla qui il codice di sincronizzazione" /><div className="btn-row"><button className="btn" onClick={importShareCode}><Download size={16} /> Importa dati</button><button className="btn btn-outline" onClick={()=>setSyncText(data.sync?.shareCode || '')}>Usa ultimo codice</button></div>{syncStatus && <div className="info-box">{syncStatus}</div>}<div className="two-col"><InfoBox title="Ultimo export" text={formatDateTime(data.sync?.lastExportAt)} /><InfoBox title="Ultimo import" text={formatDateTime(data.sync?.lastImportAt)} /></div></div></section>}
          />
        )}
...
function labelForTab(tab) {
  return {
    oggi: 'Oggi', classi: 'Classi', orario: 'Orario', lezioni: 'Lezioni', verifiche: 'Verifiche', scadenze: 'Scadenze', sync: 'Sync', classe: 'Vista classe'
  }[tab];
}
